...
struct {
        ...
        __uint(max_entries, 1);
        __type(key, __u32);
        __type(value, struct nodeproxy_info);
        __uint(pinning, LIBBPF_PIN_BY_NAME);
} nodeproxy_info SEC(".maps");
...
static __inline struct nodeproxy_info * get_nodeproxy_info()
{
    uint32_t key = 0;
    return bpf_map_lookup_elem(&nodeproxy_info, &key);
}
...
int app_outbound(struct __sk_buff *skb)
{
    ...
    struct iphdr *iph;
    struct udphdr *udph;
    struct nodeproxy_info *np = get_nodeproxy_info();
    ...
    capture_dns = np->flag & CAPTURE_DNS_FLAG;
    ...
    __builtin_memcpy(eth->h_dest, np->mac_addr, ETH_ALEN);

    skb->cb[4] = OUTBOUND_CB;

    dbg("outbound redirect to nodeproxy ifidx: %u\n", np->ifindex);
...
int app_inbound(struct __sk_buff *skb)
{
    ...
    np = get_nodeproxy_info();

    if (!np || np->ifindex == 0) {
        dbg("inbound_to_nodeproxy unable to retrieve nodeproxy_info\n");
        return TC_ACT_OK;
    }
    ...
    dbg("inbound redirect to nodeproxy ifidx: %u\n", np->ifindex);
    return bpf_redirect(np->ifindex, 0);
}
...
int nodeproxy_host_ingress(struct __sk_buff *skb)
{
    ...
    dbg("nodeproxy redirect to app(0x%x) ifidx: %u\n", bpf_ntohl(iph->daddr), pi->ifindex);

    return bpf_redirect(pi->ifindex, 0);
}

// For nodeproxy pod veth pair(in pod ns) ingress hook
SEC("tc")
int nodeproxy_ingress(struct __sk_buff *skb)
{
    void *data = (void *)(long)skb->data;
    struct ethhdr  *eth = data;
    void *data_end = (void *)(long)skb->data_end;
    ...
    if (skb->cb[4] == OUTBOUND_CB) {
        ...
        if (!skip_mark) {
            skb->mark = NODEPROXY_OUTBOUND_MARK;
        ...
        if (sk) {
            if (sk->state != BPF_TCP_LISTEN
                && sk->src_port != NODEPROXY_INBOUND_PORT
                && sk->src_port != NODEPROXY_INBOUND_PLAINTEXT_PORT) {
                skip_mark = 1;
            }
            bpf_sk_release(sk);
        }
        if (!skip_mark) {
            skb->mark = NODEPROXY_INBOUND_MARK;
